...
public class LlmChatProvider implements ChatProvider {
    ...
    private static final String SYSTEM_PROMPT = """
            ...
            - 영상/음악 콘텐츠를 찾아·추천·검색해 달라는 요청(예: "주일예배 영상 찾아줘", "워십 음악 검색")은
              반드시 searchContents 도구로 검색해 결과를 안내한다(직접 지어내지 않는다).
            ...
            - 다음 중 하나라도 해당하면, 사용자에게 줄 답변 맨 끝에 정확히 [UNRESOLVED] 토큰을 붙여라
              (운영자만 보고 제거하며, 미답변 학습 큐 수집에 쓰인다):
              (1) 그레이스온 서비스와 무관한 질문(날씨·환율·일반상식·타사·잡담 등),
              (2) 도구나 제공된 지식으로 답을 찾지 못함,
              (3) 사용자가 원한 정보를 주지 못하고 거절/회피하게 되는 경우.
              실제로 도움이 되는 답을 준 경우에는 절대 붙이지 않는다.
            """;
