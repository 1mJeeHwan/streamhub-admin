...
교회 예배·미디어 플랫폼(예배 영상·찬양 음악·CCM 음반 커머스·교회찾기·후원·회원)을 실제 운영 서비스의 프로덕션 스택 그대로 재현한 풀스택 포트폴리오입니다.
하나의 Spring Boot 백엔드 위에 관리자 콘솔과 공개 사용자 사이트, 그리고 AWS 배포 파이프라인까지 올라갑니다.
> 음악은 HLS(AES-128) 스트리밍, 영상은 유튜브 임베드(다시보기) — 라이브 생중계는 아직 없습니다.
...
|---|---|---|
| `PaymentProvider` | `app.payment.provider`, `iamport.*` | `mock` → `portone`(아임포트, `IAMPORT_*`) / `toss`·`kakao`·`paypal`(키 게이트) |
...
> 예: 포트원(아임포트) 결제는 `IAMPORT_IMP`/`IAMPORT_APIKEY`/`IAMPORT_SECRET` + `PAYMENT_PROVIDER=PORTONE`만 주입하면 코드 변경 없이 실 PG가 동작합니다(`imp_uid` 결제 검증·환불 포함). 토스 샌드박스는 `PAYMENT_TOSS_*`로 동일하게 전환.
...
```
graceon/                  # 레포 루트(구 streamhub-admin) — 모노레포
