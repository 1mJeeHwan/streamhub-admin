...
export function AlbumCard({ item }: { item: AlbumListItem }) {
  const [failed, setFailed] = useState(false);
  const showImage = item.coverUrl && !failed;

  return (
    <Link href={`/albums/${item.id}`} className="block w-full">
      ...
      <div className="mt-2.5">
        <p className="ellipsis-1 text-[15px] font-bold leading-snug text-active">{item.title}</p>
        <p className="ellipsis-1 mt-0.5 text-xs font-medium text-inactive">{item.artist}</p>
        {item.price != null && (
